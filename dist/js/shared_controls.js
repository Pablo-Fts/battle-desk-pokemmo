...
function smogonAnalysis(pokemonName) {
	...
	return "https://mmoshowdown.com/dex/pokemon/" + pokemonName.toLowerCase() + "/";
}
...
function loadDefaultLists() {
	$(".set-selector").select2({
		...
		initSelection: function (element, callback) {
			...
			if (value) {
				callback({id: value, text: value, pokemon: value.split(' (')[0]});
			} else {
				var first = getFirstValidSetOption(); if (first) callback(first);
			}
		}
	});
}
...
$(document).ready(function () {
	...
	// Initialize weather tracking for starting Pokemon
	$('.poke-info').each(function(i) {
		var ability = $(this).find('select.ability').val();
		if (ability) autosetWeather(ability, i);
	});
});
